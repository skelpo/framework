...
class HttpKernel extends \Symfony\Component\HttpKernel\DependencyInjection\ContainerAwareHttpKernel implements HttpKernelInterface, TerminableInterface
{
	...
	private function handleRaw(Request $request, $type = self::MASTER_REQUEST)
	{
		...
		if (stristr($s, "::"))
		{
			...
			if (method_exists($controller[0], 'onPreDispatch'))
				$d->addListener($eventName1, array(
						$controller[0],
						'onPreDispatch' 
				));
			if (method_exists($controller[0], 'on' . ucwords($actionName) . 'PreDispatch'))
				$d->addListener($eventName2, array(
						$controller[0],
						'on' . ucwords($actionName) . 'PreDispatch' 
				));
			if (method_exists($controller[0], 'onPostDispatch'))
				$d->addListener($eventName1, array(
						$controller[0],
						'onPostDispatch' 
				));
			if (method_exists($controller[0], 'on' . ucwords($actionName) . 'PostDispatch'))
				$d->addListener($eventName2, array(
						$controller[0],
						'on' . ucwords($actionName) . 'PostDispatch' 
				));
		}
