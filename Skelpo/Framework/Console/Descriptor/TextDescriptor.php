 ...
 * file that was distributed with this source code.
 */
namespace Skelpo\Framework\Console\Descriptor;
...
class TextDescriptor extends Descriptor
{
	...
	protected function describeRoute(Route $route, array $options = array())
	{
		...
		$tableRows = array(
				...
				array(
						'Host Regex',
						('' !== $route->getHost() ? $route->compile()->getHostRegex() : '') 
				),
				array(
						'Scheme',
						($route->getSchemes() ? implode('|', $route->getSchemes()) : 'ANY') 
				...
				array(
						'Requirements',
						($route->getRequirements() ? $this->formatRouterConfig($route->getRequirements()) : 'NO CUSTOM') 
				),
				array(
						'Class',
						get_class($route) 
				...
				array(
						'Defaults',
						$this->formatRouterConfig($route->getDefaults()) 
				),
				array(
						'Options',
						$this->formatRouterConfig($route->getOptions()) 
				) 
		);
	...
	protected function describeContainerServices(ContainerBuilder $builder, array $options = array())
	{
		...
		$tableHeaders = array_merge(array(
				'Service ID' 
		), $tagsNames, array(
				'Class name' 
		));
		...
		foreach ($this->sortServiceIds($serviceIds) as $serviceId)
		{
			...
			elseif ($definition instanceof Alias)
			{
				...
				$tableRows[] = array_merge(array(
						$serviceId,
						sprintf('alias for "%s"', $alias) 
				), $tagsCount ? array_fill(0, $tagsCount, '') : array());
			}
			else
			{
	...
	protected function describeContainerDefinition(Definition $definition, array $options = array())
	{
		...
		$tableRows[] = array(
				'Public',
				$definition->isPublic() ? 'yes' : 'no' 
		);
		$tableRows[] = array(
				'Synthetic',
				$definition->isSynthetic() ? 'yes' : 'no' 
		);
		$tableRows[] = array(
				'Lazy',
				$definition->isLazy() ? 'yes' : 'no' 
		);
		...
		if ($definition->getFile())
		{
			$tableRows[] = array(
					'Required File',
					$definition->getFile() ? $definition->getFile() : '-' 
			);
		}
	...
	private function renderEventListenerTable(EventDispatcherInterface $eventDispatcher, $event, array $eventListeners, SymfonyStyle $io)
	{
		...
		foreach ($eventListeners as $order => $listener)
		{
			$tableRows[] = array(
					...
					$eventDispatcher->getListenerPriority($event, $listener) 
			);
		}
