...
class FrameworkExtension extends Extension
{
	...
	private function registerProfilerConfiguration(array $config, ContainerBuilder $container, XmlFileLoader $loader)
	{
		...
		if (isset($config['matcher']))
		{
			...
			elseif (isset($config['matcher']['ip']) || isset($config['matcher']['path']) || isset($config['matcher']['ips']))
			{
				...
				{
					$definition->addMethodCall('matchIp', array(
							$config['matcher']['ip'] 
					));
				}
				...
				{
					$definition->addMethodCall('matchIps', array(
							$config['matcher']['ips'] 
					));
				}
				...
				{
					$definition->addMethodCall('matchPath', array(
							$config['matcher']['path'] 
					));
				}
			}
	...
	private function registerRequestConfiguration(array $config, ContainerBuilder $container, XmlFileLoader $loader)
	{
		if ($config['formats'])
		{
			...
			$container->getDefinition('request.add_request_formats_listener')->replaceArgument(0, $config['formats']);
		}
	}
	...
	private function registerTemplatingConfiguration(array $config, $ide, ContainerBuilder $container, XmlFileLoader $loader)
	{
		...
		if ($container->getParameter('kernel.debug'))
		{
			...
			$container->getDefinition('templating.loader.cache')->addTag('monolog.logger', array(
					'channel' => 'templating' 
			))->addMethodCall('setLogger', array(
					$logger 
			));
			$container->getDefinition('templating.loader.chain')->addTag('monolog.logger', array(
					'channel' => 'templating' 
			))->addMethodCall('setLogger', array(
					$logger 
			));
		...
		else
		{
			...
			{
				$container->getDefinition('templating.engine.delegating')->addMethodCall('addEngine', array(
						$engine 
				));
			}
	...
	private function registerAssetsConfiguration(array $config, ContainerBuilder $container, XmlFileLoader $loader)
	{
		...
		$container->getDefinition('assets.packages')->replaceArgument(0, new Reference('assets._default_package'))->replaceArgument(1, $namedPackages);
	}
	...
	private function registerTranslatorConfiguration(array $config, ContainerBuilder $container)
	{
		...
		if ($dirs)
		{
			...
			$options = array_merge($translator->getArgument(3), array(
	...
	private function registerValidationConfiguration(array $config, ContainerBuilder $container, XmlFileLoader $loader)
	{
		...
		{
			$validatorBuilder->addMethodCall('addXmlMappings', array(
					$xmlMappings 
			));
		}
		...
		{
			$validatorBuilder->addMethodCall('addYamlMappings', array(
					$yamlMappings 
			));
		}
		...
		{
			$validatorBuilder->addMethodCall('enableAnnotationMapping', array(
		...
		if (array_key_exists('static_method', $config) && $config['static_method'])
		{
			...
			{
				$validatorBuilder->addMethodCall('addMethodMapping', array(
						$methodName 
				));
			}
		}
		...
		if (isset($config['cache']))
		{
			$container->setParameter('validator.mapping.cache.prefix', 'validator_' . $this->getKernelRootHash($container));
			
			$validatorBuilder->addMethodCall('setMetadataCache', array(
	...
	private function registerAnnotationsConfiguration(array $config, ContainerBuilder $container, $loader)
	{
		...
		if ('none' !== $config['cache'])
		{
			if ('file' === $config['cache'])
			{
				...
				$container->getDefinition('annotations.filesystem_cache')->replaceArgument(0, $cacheDir);
			}
			
			$container->getDefinition('annotations.cached_reader')->replaceArgument(1, new Reference('file' !== $config['cache'] ? $config['cache'] : 'annotations.filesystem_cache'))->replaceArgument(2, $config['debug']);
	...
	{
		$container->getDefinition('property_accessor')->replaceArgument(0, $config['magic_call'])->replaceArgument(1, $config['throw_exception_on_invalid_index']);
	}
	...
	private function registerSerializerConfiguration(array $config, ContainerBuilder $container, XmlFileLoader $loader)
	{
		...
		{
			$annotationLoader = new Definition('Symfony\Component\Serializer\Mapping\Loader\AnnotationLoader', array(
					new Reference('annotation_reader') 
			));
		...
		foreach ($bundles as $bundle)
		{
			...
			{
				$definition = new Definition('Symfony\Component\Serializer\Mapping\Loader\XmlFileLoader', array(
						realpath($file) 
				));
				$definition->setPublic(false);
			...
			{
				$definition = new Definition('Symfony\Component\Serializer\Mapping\Loader\YamlFileLoader', array(
						realpath($file) 
				));
				$definition->setPublic(false);
		...
		{
			$container->setParameter('serializer.mapping.cache.prefix', 'serializer_' . $this->getKernelRootHash($container));
			
			$container->getDefinition('serializer.mapping.class_metadata_factory')->replaceArgument(1, new Reference($config['cache']));
		}
	...
	private function registerPropertyInfoConfiguration(array $config, ContainerBuilder $container, XmlFileLoader $loader)
	{
		...
		if (class_exists('phpDocumentor\Reflection\ClassReflector'))
		{
			...
			$definition->addTag('property_info.description_extractor', array(
					'priority' => - 1000 
			));
			$definition->addTag('property_info.type_extractor', array(
					'priority' => - 1001 
			));
		}
	}
